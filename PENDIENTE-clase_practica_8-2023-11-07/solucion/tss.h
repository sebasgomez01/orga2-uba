...
// Declara la función para inicializar la TSS
void tss_init(void);

// Define una lista de TSS para tareas
extern tss_t tss_tasks[MAX_TASKS];

// Declara la función para crear una TSS para una tarea del usuario
tss_t tss_create_user_task(uint32_t cr3);

// Declara una función para crear una entrada para la GDT que referencie a la TSS pasada por parámetro, osea un descriptor de TSS
