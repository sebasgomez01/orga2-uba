...
uint8_t contar_pagos_aprobados(list_t* pList, char* usuario) {
    ...
    listElem_t* elemento = pList->first;

    if (elemento->data->aprobado == 1 && strcmp(elemento->data->cobrador, usuario) == 0) {
        result++;
    }
...
uint8_t contar_pagos_rechazados(list_t* pList, char* usuario) {
    ...
    if (elemento->data->aprobado == 0 && strcmp(elemento->data->cobrador, usuario) == 0) {
        result++;
    }
...
pagoSplitted_t* split_pagos_usuario(list_t* pList, char* usuario) {
    pagoSplitted_t* result = malloc(sizeof(pagoSplitted_t));
